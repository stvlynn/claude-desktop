...
function countUnresolved(rel){
  const source=restoredRelToSource.get(rel); if (!source) return 999;
  let count=0;
  for (const imp of getImports(source)){
    if (BUILTIN_RE.test(imp) || SAFE_EXTERNAL.has(imp)) continue;
    if (!imp.startsWith("./") && !imp.startsWith("../")){ count++; continue; }
    if (!findSrcForImport(imp,rel)) count++;
  }
  return count;
}

const dirs = ["ui/","icons/","lib/","utils/","vendor/"];
const counts = {};
for (const d of dirs) counts[d] = { zero: [], total: 0 };
for (const file of restoredFiles){
  const rel=path.relative(RESTORED_ROOT,file);
  for (const d of dirs){
    if (rel.startsWith(d)){
      counts[d].total++;
      if (!restoredToSrc.has(rel) && countUnresolved(rel)===0) counts[d].zero.push(rel);
      break;
    }
  }
}
for (const d of dirs){
  console.log(`${d}: ${counts[d].zero.length} ready / ${counts[d].total} total`);
  for (const r of counts[d].zero.slice(0,10)) console.log(`  ${r}`);
  if (counts[d].zero.length>10) console.log(`  ... and ${counts[d].zero.length-10} more`);
}
