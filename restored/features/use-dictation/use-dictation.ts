...
const DICTATION_CLEANUP_GATE = "1025755912";
const MIN_DICTATION_DURATION_MS = 250;
function useDictation(options: UseDictationOptions): UseDictationResult {
  const cleanupEnabled = readStatsigGateValue(DICTATION_CLEANUP_GATE);
  const streamingEnabled = readStatsigGateValue(
    dictationStreamingUploadHeaderValue,
  );
