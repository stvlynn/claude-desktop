...
import { parseDirectivesA, parseDirectivesL } from "../../shared/lib/parse-directives";
...
export function getSkillDisplayName(skill: SkillDefinition): string {
  const skillInterface = skill.interface as SkillInterface | null | undefined;
  return skillInterface?.displayName?.trim() || parseDirectivesA(skill.name);
}
function getDefaultPrompt(skill: SkillDefinition): string | null {
  const prompt = (skill.interface as SkillInterface | null | undefined)
    ?.defaultPrompt;
