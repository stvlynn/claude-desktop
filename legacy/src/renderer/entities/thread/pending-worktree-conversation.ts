...
import { vscodeApiH, vscodeApiN } from "../../shared/runtime/vscode-api";
import { upsertBrowserSidebarTabSource } from "../../features/browser-sidebar-open-source";
import { buildStartConversationParams } from "../../shared/lib/build-start-conversation-params";
import { setPinnedThread } from "../../shared/lib/set-pinned-thread";
...
export async function finalizePendingWorktreeConversation({
  ...
  if (entry.isPinned) {
    try {
      await setPinnedThread(
        conversationId,
        true,
        entry.pinnedBeforeThreadId ?? undefined,
      );
    } catch (error) {
      vscodeApiH.warning(
...
function removeThreadGoalDraftPastedTextAttachments(
  ...
  return entry.startConversationParamsInput.fileAttachments.filter(
    (attachment) =>
      ...
      !pastedTextAttachmentPaths.has(attachment.path),
  );
}
