...
import { createPersistedSignal } from "../../../shared/runtime/persisted-signal";
...
>(
  appScopeRoot,
  (sectionKey, { get }) =>
    (get(sidebarCollapsedSectionsSignal) as SidebarCollapsedSections)[sectionKey],
);
...
>(
  appScopeRoot,
  (listKey, { get }) =>
    (get(sidebarExpandedThreadListsSignal) as SidebarThreadListExpanded)[listKey] ===
    true,
);
...
>(
  appScopeRoot,
  (groupKey, { get }) =>
    (get(sidebarCollapsedGroupsSignal) as SidebarCollapsedGroups)[groupKey] ===
    true,
);
...
export function setSidebarSectionCollapsed(
  ...
  store.set(persistedCollapsedSectionsSignal, {
    ...(store.get(sidebarCollapsedSectionsSignal) as SidebarCollapsedSections),
...
export function toggleSidebarCollapsedGroup(
  ...
  const currentCollapsedGroups = store.get(
    sidebarCollapsedGroupsSignal,
  ) as SidebarCollapsedGroups;
...
export function setSidebarThreadListExpanded(
  ...
  const expandedLists = store.get(
    sidebarExpandedThreadListsSignal,
  ) as SidebarThreadListExpanded;
