...
type GitActionsContext = {
  ...
  cwd: string;
  hostConfig: { id: string } & Record<string, unknown>;
};

type ActiveFileTab = { tabId?: string | null } | null;
...
function LocalConversationGitActionsContent({
  ...
  const { codexWorktree, conversationId, cwd, hostConfig } =
    useScopedAtomValue<GitActionsContext>(gitActionsContextAtom);
  const activeDialog = useScopedAtomValue(activeGitDialogAtom);
  const activeTab = useScopedAtomValue<ActiveFileTab>(
    fileTabController.activeTab$,
  );
