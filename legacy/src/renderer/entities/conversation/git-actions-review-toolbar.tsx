...
import type {
  ...
  PushBlockedReason,
} from "../../shared/lib/git-action-blocked-reasons";
import type { GitWorkflowPhase } from "../../features/review/git-action-messages";
import { FormattedMessage, useIntl } from "../../shared/runtime/react-intl";
import { Button } from "../../shared/ui/button";
import { PullRequestCheckStatusIcon } from "../../features/github/pull-request-status";
import {
  deriveOpenPullRequestStatus,
  derivePullRequestCheckState,
} from "../../shared/lib/derive-pull-request-status";
import { CreatePullRequestIcon } from "../../shared/conversations/git-action-icons";
import { GitActionToolbarButton } from "./git-action-toolbar-button";
import {
  listVisibleGitActions,
  resolvePrimaryGitAction,
  type GitActionKind,
} from "../../shared/lib/git-action-availability";
import {
  activeGitDialogAtom,
...
type ActiveGitWorkflow = {
  ...
  workflow: "commit" | "create-pr" | string;
};

type GitActionsScopeValue = { cwd: string; hostId: string };

type GitActionsContext = {
  cwd: string;
  hostConfig: { id: string };
};

type PullRequestStatusResult =
  | { type: "success"; data: PullRequestToolbarStatus }
  | { type: string; data?: Partial<PullRequestToolbarStatus> };

function CommitOrPushToolbarButton({ compact }: ToolbarButtonProps) {
  const store = useScopedStore(localConversationGitActionsScope);
  const scopeValue = store.value as GitActionsScopeValue;
  const intl = useIntl();
  const commitBlockedReason = useScopedValue<CommitBlockedReason | null>(
    commitBlockedReasonAtom,
  );
  const activeWorkflow = useScopedValue<ActiveGitWorkflow | null>(
    activeGitWorkflowAtom,
  );
...
function CreatePullRequestToolbarButton({ compact }: ToolbarButtonProps) {
  ...
  const scopeValue = store.value as GitActionsScopeValue;
  const intl = useIntl();
  const blockedStep = useScopedValue<GitActionBlockedStep | null>(
    createPullRequestBlockedStepAtom,
  );
  const activeWorkflow = useScopedValue<ActiveGitWorkflow | null>(
    activeGitWorkflowAtom,
...
export function PushToolbarButton({ compact }: ToolbarButtonProps) {
  const store = useScopedStore(localConversationGitActionsScope);
  const intl = useIntl();
  const pushBlockedReason = useScopedValue<PushBlockedReason | null>(
    pushBlockedReasonAtom,
  );
  const activeWorkflow = useScopedValue<ActiveGitWorkflow | null>(
    activeGitWorkflowAtom,
...
function GitActionsToolbarActionSet({
  ...
  const createPullRequestBlockedStep =
    useScopedValue<GitActionBlockedStep | null>(
      createPullRequestBlockedStepAtom,
    );
  const pushHidden = useScopedValue<boolean>(isPushActionHiddenAtom);
  const isDetachedHead = useScopedValue<boolean>(isDetachedHeadAtom);
  const activeWorkflow = useScopedValue<ActiveGitWorkflow | null>(
    activeGitWorkflowAtom,
  );
...
function ViewPullRequestToolbarButton({
  ...
  const intl = useIntl();
  const isBrowserSidebarEnabled = useScopedValue<boolean>(
    isBrowserSidebarEnabledAtom,
  );
...
function ResolvedGitActionsReviewToolbar({
  ...
  const gitActionsContext = useScopedValue<GitActionsContext>(
    gitActionsContextAtom,
  );
  const { cwd, hostConfig } = gitActionsContext;
  const headBranch = useScopedQuery<string | null>(
    conversationHeadBranchAtom,
    gitActionsContext,
  );
  const pullRequestStatusResult = useScopedQuery<PullRequestStatusResult>(
    pullRequestStatusAtom,
    {
      cwd,
      headBranch,
  ...
  const pullRequestStatus =
    ...
    pullRequestStatusResult.data?.url != null
      ? (pullRequestStatusResult.data as PullRequestToolbarStatus)
      : null;
