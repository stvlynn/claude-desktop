...
} from "../../shared/runtime/user-message.facade";
import type {
  HookRunStats,
  ThreadDetailLevel,
  UserMessageModel,
...
export function UserMessage({
  ...
  const editDraft = useScopedAtomValue<string | null>(
    editDraftByTurnId,
    turnId,
  );
  const isEditing = canEdit && editDraft != null;
  const intl = useIntl();
  const hostContext = useSignalValue<HostContext>(hostContextSignal);
...
function UserMessageBody({
  ...
  const store = useAppStore(appScopeRoot);
  const isCodeMode = useSignalValue<boolean>(codeModeSignal);
