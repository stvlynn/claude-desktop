...
import {
  ChevronIcon,
  initChevronDownIcon,
} from "../../../shared/icons/chevron-icon";
import {
  GitBranchIcon,
  initGitBranchIcon,
} from "../../../shared/icons/git-branch-icon";
import { once } from "../../../shared/runtime/commonjs-interop";
import { initSpinnerComponent, Spinner } from "../../../shared/ui/spinner";
import {
  useScope,
  useScopedValue,
  useSignalValue,
...
} from "../../../features/worktree/host-worktree-context";
...
} from "../../../shared/conversations/local-conversation-thread-parts/thread-summary-environment-runtime";
import {
  composerModeState as composerModeSignal,
  initComposerModeRuntime,
  setComposerModeForScope,
} from "../../../features/composer/composer-view-state/index";
import type {
  ComposerMode,
  ScopeStore,
} from "../../../features/composer/composer-view-state/types";
import type { EnvironmentTerminalController } from "../../../shared/runtime/environment-terminal-runtime";
import type { DiffStatsQueryState } from "../../../shared/runtime/thread-summary-environment-runtime";
import {
  DiffStats,
  initDiffStatsChunk,
...
} from "../../../features/git/git-branch-switcher-parts/git-branch-switcher";
import {
...
function ThreadSummaryEnvironmentModeControls({
  ...
  let scope = useScope<ScopeStore>(composerScope),
    composerMode = useSignalValue<ComposerMode>(composerModeSignal),
    setComposerMode = (nextMode: ComposerMode) => {
      setComposerModeForScope(scope, null, nextMode);
    };

  let conversationRemoteState = getConversationRemoteState(conversationId),
    threadHostId =
      useScopedValue<string>(conversationHostIdSignal, conversationId) ??
      "local",
    conversationCwd = useScopedValue<string | null>(
      conversationCwdSignal,
      conversationId,
    ),
  ...
  let isThreadHandoffSummaryEnabled = useStatsigGate("1115442235"),
    conversationTitle = useScopedValue<string | null>(
      conversationDisplayTitleSignal,
      conversationId,
    ),
    threadHandoff =
...
export function ThreadSummaryEnvironmentSection({
  ...
  let routeScope = useScope(localConversationRouteScope),
    diffStatsQuery = useSignalValue<DiffStatsQueryState>(diffStatsSignal),
    environmentTerminalController =
      useSignalValue<EnvironmentTerminalController | null>(
        environmentTerminalControllerSignal,
      ),
