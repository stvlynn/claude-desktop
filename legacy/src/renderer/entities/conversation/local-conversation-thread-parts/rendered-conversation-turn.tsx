...
export function ConversationTurnRenderUnit(
  ...
  if (unit.kind === "collapsed-tool-activity") {
    ...
    renderedContent = (
      <PendingMcpToolCalls
        items={(unit as unknown as { items: readonly unknown[] }).items}
        conversationId={conversationId}
        hostId={hostId}
    ...
    renderedContent = (
      <DynamicToolCallGroup
        ...
        items={
          Array.from(
            (unit as unknown as { items: readonly unknown[] }).items,
          ) as any[]
        }
        isActive={isActive}
        showThinkingFallback={showThinkingFallback}
    ...
    renderedContent = (
      <ExplorationActivity
        items={Array.from(entry.items) as any[]}
...
function CollapsedToolActivityRenderUnit(
  ...
  const intl = useIntl();
  const analyticsConfig = useAtomValue(codexAnalyticsConfigAtom) as {
    logProductEvent?: (
      event: unknown,
  ...
  if (showThinkingHeader) {
    ...
    activeSummaryContent = (
      <ActiveToolActivityLabel
        label={activeCommandLabel as ActiveCommandLabel}
        cwd={cwd}
        hostId={hostId}
      />
    );
  ...
  const hasDetailLines = buildToolActivityDetailLines(summary, intl).length > 0;
...
function ActiveToolActivityLabel({
  ...
  const labelIcon =
    label.faviconUrl == null ? (
      <ToolActivityIcon className="icon-xs" />
    ) : (
      <FaviconImage
