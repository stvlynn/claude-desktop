...
import { useRemUnitMultiplier } from "../../../shared/lib/use-rem-unit-multiplier";
    ...
    if (activeLabel != null) {
      ...
      return (
        <span className="inline-flex min-w-0 items-center gap-1.5">
          <ExplorationActivityIcon kind={String(activeLabel.icon)} />
...
function buildExplorationActivityNodes({
  ...
  const nodes: ExplorationCountPart[] = [];
  for (const { item, key } of items) {
    ...
    if (node != null) nodes.push({ key, node });
  }
  return nodes;
}

function ExplorationActivityIcon({ kind }: { kind: string }) {
  const Icon =
    kind === "search"
      ? DocumentSearchIcon
      : kind === "folder"
        ? FolderIcon
        : FileIcon;
  return <Icon aria-hidden={true} className="icon-xs shrink-0" />;
}
    ...
    case "read":
      return renderTruncatedSummary(
        summarizeExplorationCommand({
          ...
          threadDetailLevel,
          formatSearchQuery,
        }) || `Read ${basename(parsedCmd.name ?? parsedCmd.path ?? "")}`,
      );
    case "search": {
