...
import type { AppScopeStore } from "../../../shared/boundaries/app-scope";
import type { LocalThreadRouteValue } from "../../../shared/lib/local-thread-route";
...
} from "../../../shared/runtime/app-server-request";
import { appLogger as logger } from "../../../shared/runtime/app-logger";
import {
  initConnectorAppsRuntime,
  useAppsQuery,
} from "../../../shared/runtime/connector-apps-runtime";
import {
  useConversationAgentMode,
  useConversationHostApi,
} from "../../../shared/conversations/local-conversation-thread-parts/conversation-host-runtime";
import {
  diffSourceSignal,
...
} from "../../../shared/conversations/local-conversation-thread-parts/conversation-content-runtime";
import {
...
import {
  ...
  useStatsigLayer,
} from "../../../shared/runtime/feature-gate-runtime";
import {
...
import {
  initModalRuntime,
  openScopedModal,
} from "../../../shared/runtime/modal-runtime";
import { motion } from "../../../shared/lib/motion-signal-runtime";
import { normalizeWorkspacePath } from "../../../shared/conversations/output-artifact-runtime";
import { initPathHelpersRuntime } from "../../../shared/lib/path-helpers";
import {
  createAtomSignal,
  useSignalState,
} from "../../../shared/runtime/signal-state-runtime";
import { useAutomationItems } from "../../../shared/lib/use-automation-history-items";
import { captureConversationCopy } from "../../../shared/conversations/conversation-copy";
import { useAppgenSitesEntryPointEnabled as useAppgenEndCardEnabled } from "../../../features/appgen-gating";
import type { LocalConversationThreadContentComponentProps } from "../../conversation-thread/local-conversation-thread-frame-types";
import {
...
import type { VirtualizedTurnListContracts } from "../../conversation-thread/local-conversation-virtualized-turn-list-types";
import { initIntlRuntime, useIntl } from "../../../shared/runtime/react-intl";
...
export function LocalConversationThreadContentCore({
  ...
  let scope = useScope<AppScopeStore & { value: LocalThreadRouteValue }>(
      localConversationRouteScope,
    ),
  ...
  let handleCopyCapture = useStableCallback((event: ClipboardEvent) => {
    ...
    forkConversationFromTurn = useStableCallback(
      async (targetTurnId: string) => {
        ...
        try {
          let forkedConversationId = await sendAppServerRequest<string>(
          ...
          scope
            .get<{ danger(message: string): void }>(toastSignal)
            .danger(
        ...
        createLocalConversationSearchSource({
          getConversationState: () =>
            hasConversationRef.current
              ? {
                  turns: Array.from(conversationTurnsRef.current),
                }
              : null,
          ...
          (turnContainer: Element) => {
            for (let contentUnit of turnContainer.querySelectorAll(
              ...
              if (
                ...
                return contentUnit as HTMLElement;
            return null;
          },
        );
      },
    ...
    <>
      ...
      >
        <ThreadFindNavigationRail
          ...
          onRevealItem={(item) =>
            revealThreadFindItem(item as unknown as ThreadFindRevealRequest)
          }
        />
