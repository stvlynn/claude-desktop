...
import {
  CheckMdIcon,
  initCheckmarkIcon,
} from "../../../shared/icons/check-md-icon";
...
import {
  ...
  Tooltip,
} from "../../../shared/ui/tooltip-b/index";
import {
...
function useLocalConversationEnvironmentState(
  ...
  let scope = useScope(appScope),
    ...
    setConfigValueMutation = useHostMutation<
      { success?: boolean },
      {
        key: string;
...
export function LocalConversationEnvironmentActionControls({
  ...
  let scope = useScope(appScope),
    ...
    primaryShortcut = useScopedValue<string | null>(
      localEnvironmentActionShortcutSignal,
    ...
    openScopedModal(scope, AddLocalEnvironmentActionPopoverContent, {
      ...
      onRunAction: (action) => {
        runEnvironmentAction({
          action: {
            ...action,
            id: `environmentAction${(environment.environment.actions ?? []).length + 1}`,
          },
          commandId: null,
...
function LocalEnvironmentActionMenuItemWithShortcut({
  ...
  let shortcut = useScopedValue<string | null>(
    localEnvironmentActionShortcutSignal,
    commandId,
  );
  return (
