...
function TurnDiffCompleted({
  ...
  const conversationHostId = useAtomFamilyValue<string | null>(
    conversationHostIdAtom,
  ...
  const { data: gitDirOrigins, isFetching } = useAtomFamilyValue<{
    data?: { origins: Array<{ dir: string; root?: string | null }> } | null;
    isFetching: boolean;
  }>(gitDirOriginsQueryAtom, {
  ...
  const applyPatch = useHostRequest<
    { diff: string; cwd: string; hostConfig: unknown; revert: boolean },
    import("./patch-failure-dialog").PatchActionResult
  >("apply-patch", { source: "thread_diff" });
  ...
  const runPatchAction = async (action: PatchAction, event: MouseEvent) => {
    ...
    try {
      ...
      store
        .get<{ success(message: string, options?: unknown): void }>(
          toastControllerAtom,
        )
        .success(
          intl.formatMessage(
      ...
      store
        .get<{ danger(message: string, options?: unknown): void }>(
          toastControllerAtom,
        )
        .danger(
          intl.formatMessage(
...
interface FileOpenContextMenuProps {
  children: ReactElement<{ onContextMenu?: React.MouseEventHandler }>;
  column?: number;
  cwd: string | null;
  endLine?: number;
  hostId: string;
