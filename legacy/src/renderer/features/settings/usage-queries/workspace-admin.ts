...
import { parseCodexApiErrorDetail } from "../../../shared/lib/codex-api-error";
  ...
  const query = useQuery({
    ...
    queryFn: async () =>
      resolvedAccountId == null
        ? null
        : ((await codexRequest.safeGet("/accounts/{account_id}/settings", {
            parameters: {
              path: {
                account_id: resolvedAccountId,
              },
            },
          })) as { usage_limit_increase_request?: unknown } | null),
    select: (
      response: {
...
export function useSaveWorkspaceAdminRequestMutation() {
  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      if (requestId == null) {
        return (await codexRequest.safePost(
        ...
        )) as WorkspaceAdminRequest;
      }
      return (await codexRequest.safePatch(
