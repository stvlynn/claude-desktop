...
export function toEditableMcpServerDraft(
  ...
  const stdioConfig =
    transportType === "stdio" ? (config as StdioMcpServerConfig) : null;
  const httpConfig =
    transportType === "streamable_http"
      ? (config as HttpMcpServerConfig)
      : null;
  return {
    base: {
    ...
    stdio: {
      ...
      args:
        stdioConfig?.args && stdioConfig.args.length > 0
          ? stdioConfig.args
          : [],
      env: stdioConfig?.env ? toKeyValueEntries(stdioConfig.env) : [],
      envVars:
        stdioConfig?.env_vars && stdioConfig.env_vars.length > 0
          ? stdioConfig.env_vars
          : [],
      cwd: stdioConfig?.cwd ?? "",
    },
    http: {
      url: httpConfig?.url ?? "",
      bearerTokenEnvVar: httpConfig?.bearer_token_env_var ?? "",
      httpHeaders: httpConfig?.http_headers
        ? toKeyValueEntries(httpConfig.http_headers)
        : [],
      envHttpHeaders: httpConfig?.env_http_headers
        ? toKeyValueEntries(httpConfig.env_http_headers)
        : [],
    },
  };
}
