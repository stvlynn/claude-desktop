...
import { appScopeRoot, useAppScopeValue } from "../../shared/runtime/app-scope";
...
import { useAppServerManagerForHost } from "../app-server/app-server-manager-hooks/registry";
import { useGlobalState } from "../../shared/lib/use-global-state";
import {
  ...
  type AuthMethod,
  type AuthState,
} from "../../entities/auth/use-auth-state";

export type { AuthState } from "../../entities/auth/use-auth-state";
...
function useIsCopilotApiAvailable(): boolean {
  const data = useAppScopeValue<{ available?: boolean }>(copilotApiAvailabilityQuery);
...
function useAuthForManager(
  ...
  const { data, isLoading } = useGlobalState("use-copilot-auth-if-available") as {
    data?: boolean;
    isLoading?: boolean;
  };
