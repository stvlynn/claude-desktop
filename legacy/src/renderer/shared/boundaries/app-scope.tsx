...
type SignalMetadata<T = unknown> = {
  defaultValue?: T | (() => T);
  id: string;
  scopeRead?: (key: unknown, store: AppScopeStore) => T;
};
type Signal<T = unknown> = Atom<T> & SignalMetadata<T>;
type WritableSignal<T = unknown> = PrimitiveAtom<T> & SignalMetadata<T>;
...
export class AppScopeStore {
  private readonly values = new Map<string, unknown>();
  readonly query = {
    invalidateQueries: async (_filters?: unknown): Promise<void> => {},
  ...
  get<T = unknown>(signal: unknown, key?: unknown): T {
    ...
    if (typedSignal?.scopeRead) return typedSignal.scopeRead(key, this);
...
export function appScopeL<T>(
  ...
  return Object.assign(
    atom(() => scopeRead(undefined, rootStore)),
    {
...
export function _appScopeC<T>(
  ...
  const scopeRead = (_key: unknown, store: AppScopeStore) => read(store);
  return Object.assign(
    atom(() => scopeRead(undefined, rootStore)),
    {
...
export function _appScopeY<T>(value: T): PrimitiveAtom<T> {
  return atom(value) as PrimitiveAtom<T>;
}
