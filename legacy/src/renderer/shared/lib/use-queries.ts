...
// TanStack Query-compatible useQueries shim used by connector query modules.
...
import { appScopeT, appScopeX } from "../runtime/query-observer-stubs";
import {
  vscodeApiA,
  vscodeApiB,
  vscodeApiC,
  ...
  #findMatchingObservers(queries: QueryOptions[]) {
    const previousObservers = new Map(
      this.#observers.map((observer) => [
        (observer.options as { queryHash?: string }).queryHash ?? "",
        observer,
      ]),
    );
    ...
    queries.forEach((queryOptions) => {
      const defaultedQueryOptions = (
        this.#client as {
          defaultQueryOptions: (options: QueryOptions) => QueryOptions;
        }
      ).defaultQueryOptions(queryOptions);
      const previousObserver = previousObservers.get(
        defaultedQueryOptions.queryHash as string,
      );
      observerMatches.push({
        defaultedQueryOptions,
        observer:
          previousObserver ??
          (new appScopeT(this.#client, defaultedQueryOptions) as QueryObserver),
      });
    });
...
export function UseQueries(
  ...
  const [optimisticResult, getCombinedResult, trackResult] =
    observer.getOptimisticResult(
      defaultedQueries,
      options.combine as ((results: QueryResult[]) => unknown) | undefined,
    );
  ...
  React.useSyncExternalStore(
    React.useCallback(
      (listener) => {
        ...
        return observer.subscribe(_appScopeJ.batchCalls(listener)) ?? (() => {});
      },
  ...
  const suspensePromises = optimisticResult.some((result, index) =>
    ...
    ? optimisticResult.flatMap((result, index) => {
        ...
        if (defaultedQuery) {
          ...
          const optimisticObserver = queryObserver as unknown as {
  ...
  const firstErrorResult = optimisticResult.find((result, index) => {
    ...
    return (
      ...
      vscodeShouldThrowError({
        result,
        errorResetBoundary,
        throwOnError: defaultedQuery.throwOnError as boolean | undefined,
        query: client.getQueryCache().get(defaultedQuery.queryHash as string),
        suspense: defaultedQuery.suspense as boolean | undefined,
      })
    );
  });
