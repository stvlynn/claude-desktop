...
export function useFloatingWindowPointerInteractivity({
  ...
  const getIsPaused = useStableCallback(() => isPaused?.() ?? false);
  const publishInteractiveChange = useStableCallback((isInteractive: boolean) => {
    setIsPointerNonInteractive(!isInteractive);
    onInteractiveChange(isInteractive);
  });
  const publishCleanupInteractive = useStableCallback(() => {
