...
// Context menu with Electron-native menu support and a Radix fallback.
import React, {
  type ComponentProps,
...
import { useWindowZoom } from "../lib/window-zoom-context";
import {
...
type ResolvedContextMenuSeparatorItem = {
  id: string;
  type: "separator";
  nativeLabel: "";
  submenu: undefined;
...
type ResolvedContextMenuActionItem = ContextMenuBaseItem & {
  type?: "item";
  nativeLabel: string;
  nativeTooltip?: string;
...
type ResolvedContextMenuCheckboxItem = ContextMenuBaseItem & {
  type: "checkbox";
  checked?: boolean;
  nativeLabel: string;
  nativeTooltip?: string;
...
type ResolvedContextMenuItem =
  | ResolvedContextMenuSeparatorItem
  | ResolvedContextMenuActionItem
  | ResolvedContextMenuCheckboxItem;
...
function resolveMenuItems(
  ...
  return items.map((item): ResolvedContextMenuItem => {
    if (item.type === "separator") {
      const resolved: ResolvedContextMenuSeparatorItem = {
        id: item.id,
        type: "separator",
        nativeLabel: "",
        submenu: undefined,
    ...
    const nativeLabel =
      typeof item.label === "string"
        ? item.label
        : item.nativeLabel ?? item.id;
    const nativeTooltip = item.tooltip
      ? typeof item.tooltip === "string"
        ? item.tooltip
        : item.nativeTooltip
      : item.nativeTooltip;
    const submenu = item.submenu ? resolveMenuItems(item.submenu) : undefined;
    if (item.type === "checkbox") {
      const resolved: ResolvedContextMenuCheckboxItem = {
        ...item,
        nativeLabel,
        nativeTooltip,
        submenu,
    ...
    const resolved: ResolvedContextMenuActionItem = {
      ...item,
      nativeLabel,
      nativeTooltip,
      submenu,
...
export function ContextMenu({
  ...
  const initialItems = resolveMenuItems(items);
  ...
  const updateResolvedItems = React.useCallback(
    (nextItems: ContextMenuItemDefinition[]) => {
      const nextResolvedItems = resolveMenuItems(nextItems);
  ...
  React.useEffect(() => {
    if (!getItems) {
      setResolvedItems(resolveMenuItems(items ?? []));
    }
  }, [getItems, items]);
  ...
  return items.map((item) => {
    ...
    if (item.type === "checkbox") {
      return (
        <Tooltip
          key={item.id}
          tooltipContent={item.tooltip ?? null}
          disabled={item.tooltip == null}
        >
          <ContextMenuCheckboxItem
    ...
    return (
      ...
      >
        ...
        >
          <ContextMenuItemLabel icon={item.icon} label={renderItemLabel(item)} />
        </ContextMenuItem>
      </Tooltip>
    );
  });
...
function renderItemLabel(item: ResolvedContextMenuItem): ReactNode {
  return item.type !== "separator" && item.label ? item.label : item.id;
}
