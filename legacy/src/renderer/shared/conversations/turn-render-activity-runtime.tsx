...
import { ConversationMarkdown } from "../../entities/conversation/conversation-markdown-view";
  ...
  return (
    <ConversationSummaryRow padding="offset">
      <div className="flex min-w-0 flex-col gap-1">
        ...
        >
          <ToolActivityGlyph kind={icon ?? "run-command"} />
...
function ToolActivityRow({
  ...
  return (
    <span className="inline-flex min-w-0 items-center gap-1.5 text-token-conversation-summary-leading">
      <ToolActivityGlyph kind={icon} />
      <span className="min-w-0 truncate">{label}</span>
    </span>
  );
}

function ToolActivityGlyph({ kind }: { kind?: string }) {
  const Icon =
    kind === "edit-files"
      ? EditIcon
      : kind === "mcp-tool-call"
