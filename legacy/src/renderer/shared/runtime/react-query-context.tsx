...
const fallbackQueryClient: RuntimeQueryClient = {
  ...
  async fetchQuery(options) {
    if (!options.queryFn || typeof options.queryFn !== "function")
      return getCachedData(options.queryKey);
  ...
  setQueryData<TData>(queryKey: QueryKey, value: TData | ((current: TData | undefined) => TData)): TData | undefined {
    const current = getCachedData<TData>(queryKey);
    const next =
      typeof value === "function"
        ? (value as (current: TData | undefined) => TData)(current)
        : value;
