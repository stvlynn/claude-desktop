...
import { appScopeRoot, appScopeUnderscore } from "./app-scope";
...
export function openDialog<Props extends Record<string, unknown>>(
  ...
  const entry: DialogEntry = {
    ...
    props: {
      ...
      onClose: () => {
        providedOnClose?.();
        closeDialog(store, component as React.ComponentType<unknown>);
      },
    },
  };
...
export function closeDialog(
  ...
  for (let i = dialogStack.length - 1; i >= 0; i--) {
    if (dialogStack[i].component === component) {
      index = i;
      break;
    }
  }
