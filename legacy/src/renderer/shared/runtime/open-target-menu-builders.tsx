...
import { buildExternalLinkContextMenuItems } from "../lib/external-link/browser-actions";
...
export function buildOpenTargetMenuItems(
  options: OpenTargetListOptions,
): OpenTargetMenuItem[];
export function buildOpenTargetMenuItems(
  options: ExternalLinkMenuOptions,
): ContextMenuItemDefinition[];
export function buildOpenTargetMenuItems(
  options: OpenTargetListOptions | ExternalLinkMenuOptions,
...
export function filterOpenInTargets(options: {
  ...
  const items = buildOpenTargetMenuItems(options) as OpenTargetMenuItem[];
  return items.filter(isVisibleOpenTargetMenuItem);
}
