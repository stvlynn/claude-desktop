...
// Application: register the custom `app://` protocol handler for bundled
// webviews and trusted local file access.
...
import {
  ...
  stripLeadingSlashFromWindowsDrivePath,
} from "./path-utils";
...
function toNativeFilePath(value: string): string {
  return stripLeadingSlashFromWindowsDrivePath(value);
}
...
function decodeAppFilePathname(pathname: string): string | null {
  ...
  if (!isAbsoluteFilePath(normalizedPath)) return null;
...
function rendererDevOrigin(): string | null {
  ...
  const value =
    process.env.ELECTRON_RENDERER_URL ?? process.env.RENDERER_URL ?? null;
