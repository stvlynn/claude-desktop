...
// SQLite state database backup helpers used when the database is corrupt or
...
import { isNodeErrorCode } from "./desktop-runtime-utils";
...
export async function backUpStateDatabaseFiles(
  ...
  if (directoryStats?.isFile()) return backUpDatabaseDirectory(databaseDirectory);
