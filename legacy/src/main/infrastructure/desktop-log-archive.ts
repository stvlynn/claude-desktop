...
// Creates and removes a gzipped tar archive of the current day's desktop logs
...
export type FeedbackLogArchive = {
  archiveId: string;
  archivePath: string;
};

const gzip = promisify(zlib.gzip);
const logger = createLazyScopedStructuredLogger("feedback-desktop-log-archive");
...
export async function createFeedbackDesktopLogArchive(
  now: Date = new Date(),
  ...
  const archiveDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "claude-feedback-desktop-log-archive-"),
  );
  const archivePath = path.join(
    archiveDir,
    `claude-desktop-app-logs-${formatArchiveDay(day)}.tar.gz`,
  );
...
export async function removeFeedbackDesktopLogArchive(
  archiveId: string,
): Promise<boolean> {
