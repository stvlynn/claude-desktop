...
import { colorNumberForDeviceStatus } from "../infrastructure/build-version";
...
const logger = createScopedStructuredLogger("CodexMicroService");
const colorForThreadStatus = (status: string): number =>
  colorNumberForDeviceStatus(status) ?? 0;
const {
  ConnectionEventType,
  DeviceType,
  OAILightingEffect,
