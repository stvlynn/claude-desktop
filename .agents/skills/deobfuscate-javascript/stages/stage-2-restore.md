...
- **Chunk-name lookup (high confidence):** strips the hash suffix off `../clsx-DDuZWq6Y.js` → `clsx`, looks it up in `CHUNK_NAME_REGISTRY`; if found, every specifier is rewritten. The registry knows clsx, classnames, tslib, react, react-dom, react-is, jsx-runtime, jsx-dev-runtime, use-sync-external-store, Jotai, `@dnd-kit/*`, framer-motion, react-intl, marked, `@floating-ui/*`, Statsig, and more.
