---
name: claude-app-ref-refresh
description: Refresh the local Claude app reference source from the installed macOS Claude.app asar. Use when an agent needs the latest installed Claude app code, must recreate ./ref from /Applications/Claude.app/Contents/Resources/app.asar, or must format extracted JavaScript and CSS while excluding ref/node_modules.
---

# Claude App Ref Refresh
...
1. **Sync** — deletes only `<cwd>/ref`, then extracts `/Applications/Claude.app/Contents/Resources/app.asar` into a fresh `ref` directory.
...
- The script logs `Formatting N JS/CSS file(s) with Prettier, ignoring git/prettier ignore files...`, `Prettier verification complete.`, and then `Claude app ref refresh complete.`
...
- Set `CLAUDE_APP_ASAR=/path/to/app.asar` before running when the Claude app bundle lives somewhere else.
