...
import { produceImmutableUpdate } from "../../shared/runtime/thread-context-inputs.facade";
import { sendAppServerRequest } from "../../shared/runtime/use-host-config.facade";
import { vscodeApiF, vscodeApiU } from "../../shared/runtime/vscode-api";
...
const hooksForProjectQueryOptions = createQuerySignalFamily(
  appScopeRoot,
  (key?: { cwd: string | null; hostId: string }) => {
    const { hostId, cwd } = key ?? { hostId: "", cwd: null };
    return {
...
const hooksForProjectsQueryOptions = createQuerySignalFamily(
  appScopeRoot,
  (key?: { cwds: readonly string[] | null; hostId: string }) => {
    const { hostId, cwds } = key ?? { hostId: "", cwds: null };
    return {
  ...
  (hostId?: string) => {
    ...
    onMutate: async (
      ...
      await client.cancelQueries({
        predicate: ({ queryKey }) => isHooksQueryForHost(queryKey, id),
      });
    ...
    onSettled: async (
      ...
      await invalidateHooksQueries(client, id, {
        broadcast: true,
      });
    },
  };
