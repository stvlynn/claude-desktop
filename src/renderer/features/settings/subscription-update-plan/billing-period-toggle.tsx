...
export function PlanBillingPeriodSegmentedControl({
  ...
  return (
    <div
      className={`bg-token-foreground/10 inline-grid gap-1 rounded-2xl p-1 ${className ?? ""}`}
      role="group"
    ...
    >
      {options.map((option) => {
        ...
        return (
          <button
            key={option.id}
            className={`cursor-interaction relative isolate min-w-0 rounded-xl px-3 py-1.5 text-sm font-medium transition-transform duration-150 active:scale-[0.98] ${selected ? "text-black" : "text-token-text-secondary"}`}
            type="button"
