...
// Plan management helpers for SKU comparison, display state, and purchase URLs.
...
export function getMobileSubscriptionPlatform(subscriptionSource: string) {
...
export function getCodexPlanDisplayState(plan: string) {
...
export function isPaidCodexPlan(plan: string) {
  return (
    plan === Sku.GO ||
    plan === Sku.PLUS ||
    plan === Sku.PROLITE ||
...
export function isCreditSettingsManageablePlan(plan: string) {
...
export function getCodexPlanManagementUrl({
  currentPlan,
  targetPlan,
}: {
  currentPlan: keyof typeof CODEx_PLAN_RANK;
...
export function initPlanManagementStateChunk(): void {}
export {
