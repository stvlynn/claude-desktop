...
import { parsePatchFiles } from "../parse-patch-files";
import {
...
import type { ParsedPatchContent } from "../parse-patch-files/types";
import type {
  DiffScaleSummary,
  ParsedDiffFile,
  ParsedPatchFile,
...
export function parseDiff(
  ...
  let parsedPatches: ParsedPatchContent[];
  try {
  ...
  for (const patch of parsedPatches) {
    for (const file of patch.files) {
      ...
      try {
        ...
        console.error("Failed to parse diff", { name: file.name, error });
      }
      fileIndex += 1;
    }
  }
...
export const initParseDiffRuntimeChunk = undefined as never;
