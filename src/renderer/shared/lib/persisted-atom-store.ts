...
function getPersistedAtomValue<T>(key: string, fallback: T): T {
  return createPersistedAtomStore().getItem(key, fallback) as T;
}
...
function subscribePersistedAtomValue<T>(
  key: string,
  fallback: T,
