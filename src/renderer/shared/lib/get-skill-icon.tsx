...
import * as sourcePaths from "../runtime/src-l0hb-mz-p";
...
function createCurrentColorMaskIconComponent(
  ...
  function CurrentColorMaskIcon(props: IconComponentProps) {
    let { className } = props,
      ...
      preloadMaskImage,
      preloadDeps: React.DependencyList = [];
    preloadMaskImage = () => {
...
function SkillCreatorIcon(props: IconComponentProps) {
  let { className } = props,
    ...
    electronThemeDeps: React.DependencyList = [];
  observeElectronTheme = () => {
  ...
  let observeSystemTheme,
    systemThemeDeps: React.DependencyList = [];
  observeSystemTheme = () => {
...
const EXPLICIT_SKILL_ICONS: Record<string, SkillIconComponent> = {
