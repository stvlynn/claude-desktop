...
export function useHotkey({
  ...
  const parsedChords = useMemo(() => {
    ...
    return splitAcceleratorSequence(accelerator).map((item: string) =>
      parseAcceleratorChord(item, isMacOS),
    );
  }, [accelerator]);
  const isSequence = parsedChords.length > 1;
  const keyDownActiveRef = useRef(false);
  const activeChordRef = useRef<ParsedAcceleratorChord | null>(null);
  const sequenceIndexRef = useRef(0);
  const sequenceResetTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: Event) => handleKeyDown(event as KeyboardEvent);
    const onKeyUp = (event: Event) => handleKeyUp(event as KeyboardEvent);
...
export const initUseHotkeyChunk = undefined as never;
