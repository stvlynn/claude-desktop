...
// File-tree icon resolver: maps file names and extensions to sprite tokens.
...
} from "./icon-resolver-icon-mappings";
import { fileTreeIconSpriteBySet } from "./icon-resolver-sprite-data";
import type {
...
export function hasCompleteBuiltinSprites(set: FileTreeIconSet): boolean {
  return set !== "none" && spriteSetsWithCompleteBuiltins.has(set);
}

export function getFileTreeIconSprite(set: FileTreeIconSet): string | undefined {
  return fileTreeIconSpriteBySet[
    (set === "none" ? "minimal" : set) as keyof typeof fileTreeIconSpriteBySet
  ];
}

export function createFileTreeIconResolver(
  input?: FileTreeIconResolverInput,
): FileTreeIconResolver {
