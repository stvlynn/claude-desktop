...
import { appScopeRoot as sharedObjectAppScopeRoot } from "../app-scope";
import {
  hostMessageBridge,
  initAppRuntimeChunk,
...
function subscribeToSharedObjectUpdates(
  ...
  const unsubscribeHost = hostMessageBridge.subscribe(
    ...
    (message: unknown) => {
      const typedMessage = message as SharedObjectUpdatedMessage;
      if (typedMessage.key === key) setCachedSharedObjectValue(key, typedMessage.value);
    },
  );
