...
export function appScopeU<T extends (...args: any[]) => unknown>(_scope: unknown, resolver: T): { resolve: (...args: any[]) => ReturnType<T> } {
...
export function _appScopeY<T>(value: T, ..._args: unknown[]): PrimitiveAtom<T> {
  return atom(value) as PrimitiveAtom<T>;
}

export function appScopeR({
  children,
}: AppScopeProviderProps): ReactElement {
  return <>{children}</>;
}
