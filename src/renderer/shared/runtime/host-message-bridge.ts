...
// Minimal Electron app-main host-message bridge used by shared-object state.
...
function normalizeLogTags(
  tags?: AppMainLogTags,
): { safe?: Record<string, unknown>; sensitive: Record<string, unknown> } | undefined {
  return tags == null
    ? undefined
    : {
        ...tags,
...
function createAppMainLogger(): Record<
  AppMainLogLevel,
...
function initHostMessageApi(): void {
  const api: HostMessageApi = {
    postMessage(message) {
      let forwardedViaBridge = false;
      const electronBridge = (window as WindowWithElectronBridge).electronBridge;

      if (electronBridge?.sendMessageFromView != null) {
        Promise.resolve(electronBridge.sendMessageFromView(message)).catch(
          (error: unknown) => {
            if (message.type !== "log-message") {
              appMainLogger.warning("Failed to send message from view", {
                safe: { type: message.type },
...
let hostMessageApi!: HostMessageApi;
...
export class ElectronHostMessageBridge {
  ...
  dispatchMessage(type: string, payload: Record<string, unknown> = {}): void {
    if (hostMessageApi == null) throw new Error("VS Code API is unavailable");
  ...
  deliverMessage(type: string, message: HostMessage): void {
    ...
    const dispatchMessage = (
      ...
      this.dispatchMessage(nextType, nextPayload);
    };
...
// Initialize immediately so shared-object state can dispatch without an explicit
// init call. This preserves the original lazy-chunk semantics while keeping the
// bridge available as soon as this module is imported in the renderer.
initAppRuntimeChunk();
