...
export function InlineMentionContent({
  ...
  return (
    ...
    >
      ...
      <span className={classNames("min-w-0 break-words", textClassName)}>{text}</span>
    </span>
  );
}
...
function renderInlineMentionIcon(icon: InlineMentionIcon | null | undefined) {
  ...
  if (
    ...
    return cloneElement(icon, {
      className: classNames(inlineMentionIconClass, icon.props.className),
    });
  }
  const Icon = icon;
