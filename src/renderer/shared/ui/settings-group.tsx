...
function SettingsGroupRoot({
  ...
  return (
    <section id={id} className={classNames("flex flex-col", className)}>
      {children}
    </section>
  );
}
...
function SettingsGroupHeader({
  ...
  const textStackClassName = classNames(
...
function SettingsGroupContent({
  ...
  return (
    <div className={classNames("flex flex-col gap-1.5", className)}>{children}</div>
  );
}
