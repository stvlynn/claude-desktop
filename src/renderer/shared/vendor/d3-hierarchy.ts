// @ts-nocheck
// Restored from ref/webview/assets/src-BhkLFyc4.js
// d3-hierarchy and d3-scale-chromatic helpers bundled for Mermaid diagrams.
export { hierarchy, partition, treemap, treemapSquarify } from "d3-hierarchy";
export { schemeSet2, schemeTableau10 } from "d3-scale-chromatic";

export function initD3ColorSchemesChunk() {}
export function initD3HierarchyChunk() {
  initD3ColorSchemesChunk();
}
