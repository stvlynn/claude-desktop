...
// Role-based welcome prompt suggestions shown on the home / onboarding surface.
...
export const CONNECT_APPS_ROW_MESSAGE: MessageDescriptor = defineMessage({
...
export const WELCOME_PROMPTS_BY_ROLE: Record<OnboardingRole, WelcomePrompt[]> = {
  engineering: [
    {
...
export function normalizeOnboardingRole(role: string): OnboardingRole {
  return role === "default"
    ? "engineering"
...
export function normalizeOnboardingRoles(roles: string[]): OnboardingRole[] {
  const nonEmptyRoles = roles.length > 0 ? roles : ["something_else"];
  return Array.from(new Set(nonEmptyRoles.map(normalizeOnboardingRole)));
}

export type OnboardingRolePromptSuggestions = {
  connectAppsRowMessage: MessageDescriptor;
  role: OnboardingRole;
  suggestionPrompts: WelcomePrompt[];
};

export function getOnboardingRolePromptSuggestions({
  roles,
}: {
  roles: string[];
}): OnboardingRolePromptSuggestions {
  return {
...
type SelectRoleSuggestionPromptsOptions = {
  roles: string[];
  promptsByRole: Record<OnboardingRole, WelcomePrompt[]>;
  limit?: number;
};
...
}: SelectRoleSuggestionPromptsOptions): WelcomePrompt[] {
  const promptListsByRole = normalizeOnboardingRoles(roles).map(
    (role) => promptsByRole[role],
  );
  const promptIndexesByRole = promptListsByRole.map(() => 0);
  const selectedPrompts: WelcomePrompt[] = [];
  const seenPrompts = new Set<WelcomePrompt>();

  for (const [promptListIndex, promptList] of promptListsByRole.entries()) {
    const initialPrompt = promptList[promptIndexesByRole[promptListIndex]++];
    if (!initialPrompt || seenPrompts.has(initialPrompt)) continue;
    seenPrompts.add(initialPrompt);
    selectedPrompts.push(initialPrompt);
    if (selectedPrompts.length >= maxSuggestions) return selectedPrompts;
  }

  for (; selectedPrompts.length < maxSuggestions; ) {
    let didAddPrompt = false;
    for (let roleIndex = 0; roleIndex < promptListsByRole.length; roleIndex++) {
      const nextPrompt =
        promptListsByRole[roleIndex][promptIndexesByRole[roleIndex]++];
      if (!nextPrompt || seenPrompts.has(nextPrompt)) continue;
      seenPrompts.add(nextPrompt);
      selectedPrompts.push(nextPrompt);
      didAddPrompt = true;
      if (selectedPrompts.length >= maxSuggestions) break;
    }
