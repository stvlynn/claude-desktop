...
export function createBrowserSidebarAnchorStateForEditorEvent(
  ...
  if (event.anchor.type === "text") {
    return (
      restoreBrowserSidebarTextAnchorState(
        event.anchor.value as BrowserSidebarTextAnchor,
        rootWindow,
      ) ??
      createBrowserSidebarAnchorStateForAnchor(
        event.anchor.value,
  ...
  const elementAnchor = event.anchor as Extract<
    BrowserSidebarStoredEditorAnchor,
    { type: "element" }
  >;
  const elementRect =
    elementAnchor.element == null || !elementAnchor.element.isConnected
      ? null
      : getVisibleElementViewportRect(elementAnchor.element);
  const viewportSize = getBrowserSidebarViewportSize(rootWindow);
  const anchor =
    elementAnchor.element == null || !elementAnchor.element.isConnected
      ? elementAnchor.value
  ...
  return {
    ...
    cardViewportRect:
      event.target.mode === "design"
        ? (elementAnchor.cardViewportRect ??
          (elementRect == null
            ? undefined
