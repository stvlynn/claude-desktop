...
import { createScopedStructuredLogger } from "../structured-logger";
...
import {
  ...
  type WorkerRequest,
  type WorkerRequestCancelMessage,
...
export class WorkerBusMessageHandler {
  ...
  async handleMessage(
    ...
    const inbound = message as WorkerInboundMessage & Record<string, unknown>;
    if (inbound.workerId !== this.workerId) {
    ...
    switch (inbound.type) {
      case "worker-request": {
        const requestMessage = inbound as WorkerRequestMessage;
        this.pending.set(requestMessage.request.id, webContents);
        this.onWorkerRequestSent?.();
        this.workerManager.postMessage(requestMessage);
        return;
      }
      case "worker-request-cancel": {
        const cancelMessage = inbound as WorkerRequestCancelMessage;
        this.pending.delete(cancelMessage.id);
        break;
      }
    }
    this.workerManager.postMessage(inbound);
  }
  ...
  private handleWorkerMessage(message: WorkerOutboundMessage): void {
    const outbound = message as WorkerOutboundMessage & Record<string, unknown>;
    if (outbound.workerId !== this.workerId) {
      this.logger.warning("Received worker message for wrong worker", {
        safe: {
          expectedWorkerId: this.workerId,
    ...
    if (outbound.type === "worker-response") {
      this.handleWorkerResponse(outbound as WorkerResponseMessage);
      return;
    }
    this.hostListeners.forEach((listener) => listener(outbound));
