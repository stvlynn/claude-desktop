...
// OWL (Overlay Window Layer) feature flag bootstrap cache and settings API.

import { mkdirSync, readFileSync } from "node:fs";
import * as path from "node:path";
import { app } from "electron";
import { owlFeatureNames } from "./build-version";
import {
  arrayOfStrings,
  isRecord,
  writeFileAtomically,
...
function getSupportedOwlFeatureNames(): readonly string[] | null {
  return Array.isArray(owlFeatureNames)
    ? (owlFeatureNames as readonly string[])
    : null;
}
