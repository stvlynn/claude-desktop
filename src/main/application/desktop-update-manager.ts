...
import { BuildFlavor } from "../domain/app-brand";
import { readPackageMetadataField } from "../infrastructure/package-metadata";
import { createLazyScopedStructuredLogger } from "../infrastructure/structured-logger";
import { requireFromWorkspaceBoundary } from "../infrastructure/desktop-runtime-utils";
import { windowsVersionFromBuildVersion } from "../infrastructure/build-version";
...
import { resolveUpdateCheckIntervalMs } from "../infrastructure/update-version";
...
} from "../infrastructure/desktop-updater-types";

const sparkleLogger = createLazyScopedStructuredLogger("sparkle");
...
export class DesktopUpdateManagerImpl implements DesktopUpdateManager {
  ...
  private resolveWindowsStoreBuildVersion(): string {
    ...
    try {
      return windowsVersionFromBuildVersion(appVersion);
    } catch (error) {
      sparkleLogger().warning(
