...
// Application: parse OWL relaunch policies and compute relaunch deadlines.

import { app } from "electron";
import { dateFromDateEncodedBuildVersion } from "../infrastructure/build-version";
import { isRecord } from "../infrastructure/desktop-runtime-utils";
import type { ProcessWithLinkedBinding } from "../infrastructure/desktop-runtime-types";
import type {
  OwlUpdatePolicies,
  RelaunchPolicy,
  RelaunchWindowEntry,
...
function isCurrentBuildOlderThanDays(days: number | null): boolean {
  ...
  const buildDate = dateFromDateEncodedBuildVersion(app.getVersion()) ?? null;
  return buildDate
